...
```
```{r graph_path, echo=FALSE, warning=FALSE, fig.dim = c(22, 15)}
...
analyze_series <- function(data, periods, top_n = 15, font_scale = 1) {
  library(dplyr)
  library(ggplot2)
  library(forcats)
  library(grid)

  arrow_data_all <- purrr::map_dfr(periods, function(years) {
    start <- years[1]
    end <- years[2]
    ...
    arrow_data <- data %>%
      filter(Language %in% top_langs, Year %in% c(start, end)) %>%
      select(Language, Year, `Population 31 Dec`) %>%
      group_by(Language) %>%
      summarize(
        start_year = start,
        end_year = end,
        start_pop = `Population 31 Dec`[Year == start],
        end_pop = `Population 31 Dec`[Year == end],
        .groups = "drop"
      ) %>%
      mutate(
        pop_diff = end_pop - start_pop,
        label = paste0("\u0394", pop_diff),
        growth_status = ifelse(pop_diff >= 0, "Growing", "Decreasing"),
        start_pop_zero = 0,
        end_pop_adjusted = pop_diff,
  ...
  p <- ggplot(arrow_data_all) +
    ...
    labs(
      title = paste("Language Population Trends across", length(periods), "Periods"),
      x = "Population Change",
      y = "Language",
      color = "Growth"
    ) +
    ...
    theme(
      axis.text.x = element_blank(),
      axis.text.y = element_text(size = 12 * font_scale),
      axis.title.x = element_text(size = 14 * font_scale),
      axis.title.y = element_text(size = 14 * font_scale),
      legend.title = element_text(size = 12 * font_scale),
      legend.text = element_text(size = 10 * font_scale),
      strip.text = element_text(size = 13 * font_scale, face = "bold")
    )
